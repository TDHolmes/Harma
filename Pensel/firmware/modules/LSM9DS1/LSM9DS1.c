...
#define CHECK_RET(__RETVAL__) \
    (__RETVAL__ != RET_OK ? fatal_error_handler(__FILE__, __LINE__, __RETVAL__) : 0)
...
void LSM9DS1_AGINT1_ISR(void)
{
    ...
    ret_t ret = RET_OK;
    schedule_id_t sched_id; // we don't need to know the schedule ID...
    ...
    // TODO: do a quick read of status here if timing critical? or leave in the DRDY non-ISR context
    // handlers?
}
