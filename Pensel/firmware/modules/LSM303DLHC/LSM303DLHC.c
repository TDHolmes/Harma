...
static void priv_accel_normalize(accel_raw_t *raw_pkt, accel_norm_t *norm_pkt_ptr,
                                 bool adjust_gain);
...
static const float MagGainOffsets_XY[] = {0.9091f, 1.1696f, 1.4925f, 2.2222f,
                                          2.5000f, 3.0303f, 4.3478f};
//! Mag gain offsets for the Z axis to normalize to miligauss
static const float MagGainOffsets_Z[] = {1.0204f, 1.3158f, 1.6667f, 2.5000f,
                                         2.8169f, 3.3898f, 4.8780f};
 ...
 *
 * @param accel_datarate (accel_ODR_t): Datarate at which the accelerometer module will produce
 * data.
                      ...
                      mag_ODR_t mag_datarate, mag_sensitivity_t mag_sensitivity)
{
    ...
    retval =
        I2C_writeByte(ACCEL_ADDRESS, ACCEL_CTRL_REG1_A, (accel_datarate << 4) | (0b0111), true);
    if (retval != RET_OK) {
        return retval;
    }

    // Set accel sensitivity & turn on high res mode
    retval = I2C_writeByte(ACCEL_ADDRESS, ACCEL_CTRL_REG4_A, (accel_sensitivity << 4) | 0b00001000,
                           true);
    if (retval != RET_OK) {
        return retval;
    }
 ...
 */
void LSM303DLHC_drdy_ISR(void) { LSM303DLHC.data_ready = true; }
 ...
 * @retval (uint32_t): Number of queue overwrites.
 */
uint32_t LSM303DLHC_mag_packetOverwriteCount(void) { return LSM303DLHC.mag_queue.overwrite_count; }
 ...
 * @retval (uint8_t): Number of hardware overwrites that ocurred on the LSM303DLHC.
 */
uint32_t LSM303DLHC_mag_HardwareOverwriteCount(void) { return LSM303DLHC.mag_hw_overwrite_count; }
 ...
 */
ret_t rpt_LSM303DLHC_enableStreams(uint8_t *in_p, uint8_t in_len, uint8_t *UNUSED_PARAM(out_p),
 ...
 *          4. mag_sensitivity_t
 */
ret_t rpt_LSM303DLHC_changeConfig(uint8_t *in_p, uint8_t in_len, uint8_t *UNUSED_PARAM(out_p),
                                  uint8_t *UNUSED_PARAM(out_len_ptr))
{
    ret_t retval;
    mag_ODR_t mag_ODR;
    ...
    if (in_len != (sizeof(accel_ODR_t) + sizeof(accel_sensitivity_t) + sizeof(mag_ODR_t) +
                   sizeof(mag_sensitivity_t))) {
        return RET_INVALID_ARGS_ERR;
    }
    ...
    mag_sensitivity = (mag_sensitivity_t)
        in_p[sizeof(accel_ODR_t) + sizeof(accel_sensitivity_t) + sizeof(mag_ODR_t)];
 ...
 */
ret_t rpt_LSM303DLHC_setGainAdjustFlag(uint8_t *in_p, uint8_t in_len, uint8_t *UNUSED_PARAM(out_p),
                                       uint8_t *UNUSED_PARAM(out_len_ptr))
{
    if (in_len != 1) {
        return RET_INVALID_ARGS_ERR;
