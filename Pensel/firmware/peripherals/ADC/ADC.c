...
ret_t ADC_init(void)
{
    ...
    AdcHandle.Init.ClockPrescaler =
        ADC_CLOCK_SYNC_PCLK_DIV2; /* Synchronous clock mode, input ADC clock divided by 2*/
    ...
    AdcHandle.Init.ContinuousConvMode = DISABLE; /* Continuous mode disabled (no automatic
                                                    conversion restart after each conversion) */
    ...
    AdcHandle.Init.ExternalTrigConv =
        ADC_SOFTWARE_START; /* Software start to trig the 1st conversion manually, without external
                               event */
    ...
    AdcHandle.Init.DMAContinuousRequests =
        ENABLE; /* ADC DMA continuous request to match with DMA circular mode */
    AdcHandle.Init.Overrun = ADC_OVR_DATA_OVERWRITTEN; /* DR register is overwritten with the last
    ...
    sConfig.SamplingTime =
        ADC_SAMPLETIME_61CYCLES_5;          /* Sampling time (number of clock cycles unit) */
...
{
    if (HAL_ADC_Start_DMA(&AdcHandle, (uint32_t *)ADC_converted_data, ADC_NUM_SAMPLES) != HAL_OK) {
        return RET_GEN_ERR;
    } else {
        gADC_sampling = true;
        return RET_OK;
